  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.message || "Failed to create hall post")
      }
    } catch (error) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                className="bg-black border-gray-700 min-h-[200px]"
                required
              />
            </div>
            ...
            <div className="space-y-2">
              <label htmlFor="doxLink" className="text-sm font-medium block">
                Dox Link (optional) - Paste ID or Name
              </label>
              <Input
                id="doxLink"
                value={doxLink}
                onChange={(e) => setDoxLink(e.target.value)}
                placeholder="Enter paste ID or name"
                className="bg-black border-gray-700"
              />
              <p className="text-xs text-gray-400">
                Enter a paste ID (e.g., "my-paste") or paste title to link to an existing paste
              </p>
            </div>
            ...
            <div className="flex justify-between pt-4">
              ...
              <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white" disabled={isSubmitting}>
                {isSubmitting ? "Creating..." : "Create Hall Post"}
              </Button>
            </div>
          </form>
        </div>
